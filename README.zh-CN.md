...
- **🔎 智能体式论文检索。** 一个逐篇论文的子智能体会沿着每篇论文的章节目录进行导航（而非盲目的 top-k）；一个旗舰模型在原始被引文本块之上跨论文进行综合。
- **🧷 Citation Canvas（引用画布）。** 行内的 `[chunk:N]` 标记会链接回确切的段落——点击即可在渲染后的 HTML *以及*源 PDF 中同时高亮它。没有无依据的论断。
- **🌍 用你的语言作答。** 用中文提问，得到中文回答——引用照样保留。一条被记住的「始终用 X 语言回复」偏好会覆盖逐回合的语言检测。
- **📊 用大白话查看文献库统计。**「我有多少篇论文？」→ 一个 `library_stats` 智能体在一张表的白名单上运行**只读** SQL，自我修复，并用具体数字*以及*它运行过的 SQL 来作答。
- **🧠 会话级 + 全局记忆。** 被记住的事实/偏好可以按聊天保存，也可以处处保存——带有安全闸门（拒绝记录密钥）、LLM 冲突**取代**机制，以及一个可查看/编辑/(取消)启用的记忆管理面板。
- **🧭 可见的路由 + 追踪。** 一个徽标显示每个回合由哪个智能体 + 模型处理；一个可展开的追踪面板会从 SQLite 回放每一个模型/MCP/流水线步骤。
- **🌐 通过 Web + Semantic Scholar 发现论文。** `paper_search` 甚至能把含糊的指代（「那篇人人都引的扩散模型论文」）解析为一个可引用的结果。
- **📎 带上你自己的论文。** 通过 arXiv ID、URL 或上传 PDF 添加——去重并缓存；一个后台 **Marker** 工作进程会把 PDF 升级为真实的图片 + 题注 + 公式转 LaTeX。
- **🖼️ 会议级幻灯片。** 生成一份有据可依的 **Beamer 演示文稿**，它**绝不引用不存在的图**。演讲备注是一个**可选**、任意语言的后续操作；通过聊天**对单张幻灯片做差异化编辑**（「让第 3 页更简洁」）——绝不整体重新生成。针对**当前屏幕上的幻灯片**提问（「解释这张图」），它会从论文中作答，**而不改动演示文稿**。
- **🔱 分叉与回溯。** 从你过往的任意一条消息分叉出一个新聊天——原始内容保持完整。被分叉的消息会**预填充、可编辑、不自动发送**：编辑它以重新提问，或原样发送以重试。分叉会带上已启用的参考文献、会话记忆以及演示文稿，并在侧边栏中**嵌套在其父级之下**。
- **➗ 数学公式渲染。** 回答中的 LaTeX（`$…$`、`$$…$$`）会通过 KaTeX 渲染为真实公式。
- **💾 在任意设备上接着用。** 会话及其完整聊天记录存在后端，而非浏览器——在任何地方打开应用都能用。删除一个聊天会处处删除（带撤销功能）。
- **🔌 原生 MCP。** 智能体自身的工具通过 MCP（`/mcp`）提供服务；外部客户端（Claude Desktop、Cursor）可触达同一接口。
...
- **[系统需求规格说明书（System Requirements Specification）](docs/superpowers/specs/2026-05-17-paperhub-srs.md)**——权威的架构、模式、范围和验收标准（已交付至 **v2.31**）。
